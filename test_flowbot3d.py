...
def predict_flow_gtmask(sim, observation, model, reverse):
    ...
    pred_flow = model.predict(
        ...
        torch.from_numpy(np.ones(xyz.shape[0], )).float(),
    )
...
def run_test(model, category_type, category_name, instance_type):
    ...
    for id in trange(test_num):
        ...
        if scene_state['joint_states'][sim.selected_joint]['init_val'] < scene_state['joint_states'][sim.selected_joint]['cur_val']:
            print('Reversing flow prediction dir')
            reverse = True
        else:
            reverse = False
        pred_flow, segmask, pcd, xyz = predict_flow_gtmask(sim, observation, model, reverse)
        flow_norm_allpts = np.linalg.norm(pred_flow, axis=1)
        max_flow_idx = np.argpartition(flow_norm_allpts, -1)[-1:]
        ...
        max_flow_pt = pcd[segmask][max_flow_idx].reshape(1, 3)
        action = project_pts_to_2d(max_flow_pt, sim.cam_view_matrix, sim._scene_cam_intrinsics).reshape(3,)
        action = action[:2]
        ...
        for step in range(1, max_step_num + 1):
            ...
            pred_flow, segmask, pcd, xyz = predict_flow_gtmask_after(sim, observation, model, reverse)
            ...
            if 0 in pred_flow[max_flow_idx].shape:
                ...
                action = pred_flow[max_flow_idx].reshape(3, )
    ...
    if True: # manipulationfor id in range(test_num):
        ...
        for id in range(test_num):
            if id in seq_with_correct_position:
                final_result += results[f'dist2target-{id}-{max_step_num}'] / (1e-5+results[f'dist2target-{id}-{0}']) / test_num
                res_file = open(
                    os.path.join(result_dir, "umpnet_all.txt"), "a"
                )
                number = results[f'dist2target-{id}-{max_step_num}'] / (1e-5+results[f'dist2target-{id}-{0}'])
                if number > 1.0:
                    number = 1.0
