...
class PybulletSim:
    def __init__(self, gui_enabled, action_distance):
        ...
        self._scene_cam_focal_length = (
            float(self._scene_cam_image_size[1]) / 2
        ) / np.tan((np.pi * self._scene_cam_fov_w / 180) / 2)
        self._scene_cam_fov_h = (
            math.atan(
                (float(self._scene_cam_image_size[0]) / 2)
                / self._scene_cam_focal_length
            )
            * 2
            / np.pi
        ...
        self._scene_cam_projection_matrix = self.bc.computeProjectionMatrixFOV(
            ...
            aspect=float(self._scene_cam_image_size[1])
            / float(self._scene_cam_image_size[0]),
            nearVal=self._scene_cam_z_near,
            farVal=self._scene_cam_z_far,
    ...
    def get_scene_cam_data(self, cam_position, cam_lookat, cam_up_direction):
        ...
        rgb_pixels = np.array(camera_data[2]).reshape(
            (self._scene_cam_image_size[0], self._scene_cam_image_size[1], 4)
        )
        color_image = rgb_pixels[:, :, :3].astype(np.float32) / 255.0
        z_buffer = np.array(camera_data[3]).reshape(
            (self._scene_cam_image_size[0], self._scene_cam_image_size[1])
        )
        depth_image = (2.0 * self._scene_cam_z_near * self._scene_cam_z_far) / (
            self._scene_cam_z_far
            + self._scene_cam_z_near
            - (2.0 * z_buffer - 1.0) * (self._scene_cam_z_far - self._scene_cam_z_near)
        )
        segmentation_mask = np.array(camera_data[4], int).reshape(
            self._scene_cam_image_size
        )
    ...
    def get_init_boundary(self):
        ...
        if (
            joint_state["last_val"] > joint_state["init_val"]
            and joint_state["cur_val"] < joint_state["init_val"] + threshold
        ):
            reach_init = True
        if (
            joint_state["last_val"] < joint_state["init_val"]
            ...
            reach_init = True
        if (
            joint_state["cur_val"] < joint_state["init_val"] + threshold
            and joint_state["cur_val"] > joint_state["init_val"] - threshold
        ):
            reach_init = True
    ...
    def get_suction_target_position(self):
        ...
        suction_position = np.array(
            self.bc.multiplyTransforms(
                link_pos,
                link_quat,
                self.transform_start_to_link[0],
                self.transform_start_to_link[1],
            )[0]
        )
    ...
    def _move_to(self, target_position, speed_ratio=1, sleep=False):
        ...
        for i in range(step_num):
            ...
            link_pos, link_quat = link_state[0], link_state[1]
            target_direction = (
                np.array(self.bc.getMatrixFromQuaternion(link_quat)).reshape([3, 3])
                @ self.normal_to_link.T
            )
            ...
            current_position = (
                np.array(
                    [
                        self.bc.getJointState(self._suction_gripper, joint_id)[0]
                        for joint_id in [0, 1, 2]
                    ]
                )
                + self._mount_base_position
            )
    ...
    def get_observation(self):
        ...
        (
            ...
            self.cam_view_matrix,
        ) = self.get_scene_cam_data(
            self.cam_position, self.cam_lookat, self.cam_up_direction
        )

        xyz_pts, color_pts, segmentation_pts = utils.get_pointcloud(
            self.depth_image,
            self.color_image,
            self.segmentation_mask,
        ...
        # crop (remove plane)
        cropped_pcd = pcd.crop(
            o3d.geometry.AxisAlignedBoundingBox(
                min_bound=np.array([-np.inf, -np.inf, 0.005]),
        ...
        self.pcd = np.concatenate(
            [xyz_pts_sample, color_pts_sample, normal_pts_sample], axis=1
        ).astype(np.float32)
    ...
    def get_gt_position(self):
        while True:
            ...
            if (
                int(self.body_id_pts[random_idx]) == self.body_id
                and int(self.link_id_pts[random_idx]) in self.joint_states
            ):
                break
        ...
        **kwargs,
        ...
        if scene_state is None:
            ...
            (
                ...
                scaling,
                moveable_link_dict,
            ) = sim_utils.fetch_mobility_object(
                category_type, instance_type, category_name, instance_id
            )
            self.base_orientation = self.bc.multiplyTransforms(
                [0, 0, 0],
                self.bc.getQuaternionFromEuler(
                    [0, 0, np.random.rand() * np.pi / 2 - np.pi / 4]
                ),
                [0, 0, 0],
                orientation,
            )[1]
            ...
            self.category_name, self.instance_id = (
                scene_state["category_name"],
                scene_state["instance_id"],
            )
        ...
        if self.category_name == "Toy":
            urdf_flag = (
                urdf_flag
                and self.bc.URDF_USE_SELF_COLLISION_INCLUDE_PARENT
                and self.bc.URDF_USE_SELF_COLLISION
            )
        ...
        if scene_state is None:
            ...
            self.selected_joint = (
                np.random.choice(list(self.joint_states.keys()))
            ...
            for joint_id in self.joint_states.keys():
                ...
                if rand_float < 0.15:
                    ...
                    )  # hack: make sure the door is not fully closed
                elif rand_float < 0.3:
                    cur_val = max_val
                else:
        ...
        for joint_id in self.joint_states.keys():
            self.bc.resetJointState(
                self.body_id, joint_id, self.joint_states[joint_id]["init_val"]
            )
        if self.category_name == "Toy":
            x_distance = np.abs(
                self.toy_joint_vals[self.instance_id][:, 0]
                - self.joint_states[2]["init_val"]
            )
            self.bc.resetJointState(
                self.body_id,
                1,
                self.toy_joint_vals[self.instance_id][np.argmin(x_distance), 1],
            )
        ...
        if self.with_goal_state:
            self.bc.resetJointState(
                self.body_id,
                self.selected_joint,
                self.joint_states[self.selected_joint]["cur_val"],
            )
            if self.category_name == "Toy":
                x_distance = np.abs(
                    self.toy_joint_vals[self.instance_id][:, 0]
                    - self.joint_states[2]["cur_val"]
                )
                self.bc.resetJointState(
                    self.body_id,
                    1,
    ...
    def step(self, action, **kwargs):
        ...
        if action_type == 0:
            ...
            self.transform_start_to_link = self.bc.multiplyTransforms(
                vec_inv, quat_inv, position_start, [0, 0, 0, 1]
            )
            self.normal_to_link = (
                np.array(self.bc.getMatrixFromQuaternion(quat_inv)).reshape([3, 3])
                @ np.array(surface_normal).T
            )
            ...
            if self.gui:
                self.bc.addUserDebugLine(
                    lineFromXYZ=position_start,
                    lineToXYZ=position_start
                    + direction_vec * self._action_distance * 20,
                    lineColorRGB=(0, 0, 1),
                )
                self.bc.addUserDebugLine(
                    lineFromXYZ=position_start,
                    lineToXYZ=position_end,
                    lineColorRGB=(0, 1, 1),
                )
...
if __name__ == "__main__":
    ...
    for i in range(5):
        ...
        observation, reward, (reach_init, reach_boundary), info = sim.step(
            [1, direction[0], direction[1], direction[2]]
        )
